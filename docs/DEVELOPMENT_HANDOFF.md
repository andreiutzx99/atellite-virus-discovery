...
Prepared 2026-09-25 for Milestone 3 review from GitHub `main` baseline `9077fe0d2ad367f11a1fc87681b6f3c56a4de2f`. GitHub repository: [andreiutzx99/atellite-virus-discovery](https://github.com/andreiutzx99/atellite-virus-discovery). Distribution: `satellite-discovery`, version `0.3.0`, Python 3.11 or newer. Milestone 1 is merged as PR #12; Milestone 2 is merged as PR #14. Milestone 3 is committed locally on `feature/milestone-3-generic-assembly`, but publication is blocked; no remote feature ref or PR exists. GitHub `main` remains at the stated baseline.

## Read this first

This is a working metadata/download/QC application with descriptive reviews of independently supplied artifacts, an allowlisted review workflow, and isolated artificial runtime diagnostics. Milestone 2 adds provenance-rich per-record reference imports and a registered ENA/NCBI acquisition path. Milestone 3 adds registered SPAdes/Tadpole assembly for supplied single- or paired-end FASTQ, with validated canonical contigs, preserved raw output, provenance and verified reuse. Assembly remains a generic file-processing step: it is **not** a complete or scientifically validated discovery application and does not perform biological interpretation.

Milestone 1 added a trusted stage registry, a generic bounded external-tool adapter contract, eight explicit stage states, an external-module placeholder and one harmless text adapter. Milestone 2 adds immutable per-record reference snapshots and comparisons, plus a trusted provider registry with ENA primary acquisition and a conditional NCBI SRA Toolkit fallback. Milestone 3 extends the registry with a command-safe generic assembly stage. Existing QC/results and earlier workflow stages remain intact. No ViReMa, DVG analysis, candidate discovery, biological ranking or interpretation is implemented.
...
The full optional-runtime suite passed 277 tests in this environment, including real SPAdes/Tadpole single- and paired-layout fixtures. No candidate discovery, ViReMa, DVG analysis, ranking or biological interpretation was added. The [assembly guide](ASSEMBLY.md) documents configuration, inputs, outputs and limitations. GitHub PR/CI review is pending because writes through the connected GitHub API were blocked by Cloudflare at the Replit proxy; no merge occurred.

## Remaining engineering and scientific gaps

Engineering gaps still present include a unified acquisition-to-review workflow, complete dependency pinning/fingerprinting, stronger OS resource containment, broader platform deployment validation and permanent comprehensive runtime-evidence retention. Assembly execution is now registered, but software execution and artificial fixtures do not establish biological assembly quality or discovery capability.
...
Verified development baseline before Milestone 1: `c5b5eb15d737601b48009a090faef7717c92d54a`; baseline reproduction was 229 passing tests with no skips. Milestone 1 PR #12 merged as `96b6d057d5dcda7c7f1862a3696ef3cae1eb024c`; its recorded main-branch run `36137959715` passed all five jobs. Milestone 2 PR #14 is merged as `9077fe0d2ad367f11a1fc87681b6f3c56a4de2f1`, which remains the current remote `main`.

Milestone 3 is committed locally but not published. Local Git push failed with invalid credentials. GitHub reads confirm the connected OAuth is healthy, has `repo` scope, and has push/admin repository permission; both `proxyFetch` and the SDK write attempt were blocked by Cloudflare with HTTP 403 at the Replit proxy. Read-only verification confirmed no remote feature branch, PR or CI run exists, and `main` is unchanged. This is a publication blocker, not an authorization-scope finding.

The [Actions tests history](https://github.com/andreiutzx99/atellite-virus-discovery/actions/workflows/tests.yml) provides commit-specific run status; no Milestone 3 run exists yet. Authoritative published source remains GitHub `main` at `9077fe0d2ad367f11a1fc87681b6f3c56a4de2f1`; the local feature branch is not present there. Local environments, ignored data/tools, expiring Actions artifacts and historical prototypes are not part of the released source. The registry is an infrastructure extension point only; it does not establish end-to-end scientific functionality.

## Transfer brief for another platform

> GitHub `main` currently contains Milestones 1 and 2 through `9077fe0d2ad367f11a1fc87681b6f3c56a4de2f1`. The Milestone 3 implementation is committed only in the Replit feature branch because GitHub publication was blocked; it is not available from GitHub yet. If continuing from this workspace, read `docs/DEVELOPMENT_HANDOFF.md`, `docs/EXTERNAL_TOOL_ADAPTERS.md`, `docs/ASSEMBLY.md` and `docs/APPLICATION_CAPABILITY_AUDIT.md`. Milestone 3 adds a registered SPAdes/Tadpole stage for supplied FASTQ, not biological discovery. Workflow configuration cannot supply commands or imports. No ViReMa, DVG analysis, candidate discovery, ranking or interpretation is implemented. Preserve existing user data and completed QC. Distinguish software behavior, dependencies, missing engineering and scientific validation; do not interpret skipped or unavailable work as negative evidence. Do not start further milestones before review.
