...
This is a working metadata/download/QC application with descriptive reviews of independently supplied artifacts, an allowlisted review workflow, and isolated artificial runtime diagnostics. Milestone 2 adds provenance-rich per-record reference imports and a registered ENA/NCBI acquisition path. Milestone 3 adds registered SPAdes/Tadpole assembly for supplied single- or paired-end FASTQ. Milestone 4 connects validation, assembly, canonical FASTA, exact sequence catalogues, declared reference comparison, occurrence/control summaries, and consolidated reports through typed, checksummed handoffs and verified reuse. This remains generic artifact processing: it is **not** a complete or scientifically validated discovery application and does not perform biological interpretation.

Milestone 1 added a trusted stage registry, a generic bounded external-tool adapter contract, eight explicit stage states, an external-module placeholder and one harmless text adapter. Milestone 2 adds immutable per-record reference snapshots and comparisons, plus a trusted provider registry with ENA primary acquisition and a conditional NCBI SRA Toolkit fallback. Milestone 3 extends the registry with command-safe generic assembly. Milestone 4 adds artifact contracts, cross-stage compatibility checks, cache-aware preflight, top-level workflow provenance and consolidated human-/machine-readable reports. Existing QC/results and earlier workflow stages remain intact. No ViReMa, DVG analysis, candidate discovery, biological ranking or interpretation is implemented.
...
| Stage registry and review orchestration | `artifact_workflow.py`, `stage_registry.py` | Deterministic trusted-code registry includes the generic assembly stage, reference-record import, harmless external adapter and declarative external-module placeholder. Workflow JSON cannot import modules or provide commands. Ordered execution and verified earlier-stage artifacts remain supported. |
| Typed workflow artifacts | `artifact_contracts.py`, `stage_registry.py`, `artifact_workflow.py` | Validates declared file formats and metadata, enforces registered input/output contracts before execution, binds stage handoffs to checksums, and records provenance without assigning scientific meaning. |
| Workflow stage handoffs and reports | `artifact_stage_handlers.py`, `artifact_workflow.py` | Connects validated FASTQ, assembly, catalogue, reference snapshot/record import, supplied-reference BLAST, and occurrence/control reporting. The consolidated stage emits `report.json` and `report.html`; blocked/failed stages remain explicit. |
| External-tool adapter | `external_tool.py`, `assembly_adapters.py`, `example_transform_adapter.py` | Required/optional input declarations, executable/version/hash checks, safe argv execution, timeout and output limits, separate logs, checksummed output inventory, provenance, failure states and verified reuse. The assembly adapters are registered SPAdes/Tadpole implementations; the example remains an artificial text transform. |
| Provenance | `reproducibility.py`, `artifact_workflow.py` | Top-level workflow ID/schema/configuration digest, Git/runtime, stage order/graph, input/output contract descriptors, dependencies, statuses/reuse, warnings/failures, reference-record snapshot IDs and final report paths. Not a complete dependency lock or scientific certification. |
...
| Assembly runtime and adapters | `assembly_adapters.py`, `scripts/test_tadpole_runtime.py`, `scripts/check_tadpole_ci.py`, `artificial_spades.py`, `scripts/check_spades_ci.py` | Registered arbitrary-input SPAdes/Tadpole stages accept declared single/paired FASTQ, validate canonical contigs, preserve raw output and verify reuse. The original fixed artificial diagnostics remain as independent regression/runtime checks. |
...
Declared FASTQ files
  -> registered SPAdes or Tadpole assembly stage
  -> validated canonical contigs and retained raw outputs
  -> optional existing FASTA catalogue
...
| Java and BBTools | Tadpole diagnostic and generic assembly stage | BBTools 40.01 is pinned and checked against its archive before execution. Tadpole uses a bounded Java process and is available only where the verified runtime is installed. |
| Compatible SPAdes | SPAdes diagnostic and generic assembly stage | SPAdes 4.1.0 was exercised with generated input in this environment. No native Windows SPAdes support claim. |
...
`check_installed.py` tests installed package resources/entry points and fixed artificial review/reuse/provenance operations. It does not use completed user QC runs. The Milestone 1 baseline was 246 collected tests; Milestone 2 raised the suite to 259. The current Milestone 3 suite has 277 tests. In this environment, the full optional-runtime run passed all 277 tests, including real single- and paired-layout SPAdes and Tadpole runs; the installed-package check, compilation and diff checks also passed. These are software/runtime checks, not biological-quality validation. Fake executable tests exercise the NCBI command wrapper, bounds, output checks and failure handling; they do not prove a real SRA Toolkit run. The optional-tools CI job does not install SRA Toolkit.
...
## Milestone 3: generic registered assembly

The `assembly` stage accepts only the registered `spades` or `tadpole` assembler IDs and an explicit `single-end` or `paired-end` layout. Configuration cannot provide commands, imports or arbitrary tool arguments. The adapter validates FASTQ structure, base/quality characters, paired record counts, identifiers and mate roles before execution. Compressed `.fastq.gz` and `.fq.gz` inputs are streamed.

SPAdes is an external executable dependency; Tadpole uses the locally verified BBTools 40.01 runtime and Java. Both use bounded argument-list execution, tool-level thread/memory settings, a timeout and a stage-output budget. These are cooperative/tool-level controls, not hard OS CPU or memory quotas. Raw assembler output is retained. A separate canonical FASTA is validated for the existing catalogue contract, and the report distinguishes software execution from biological validation.

The assembly manifest records input hashes and names, adapter/tool/runtime identity, configuration, safe command representation, timestamps, duration, exit state, contig summary, output inventory/hashes, warnings/errors and stated resource limits. Symlinks and special outputs are rejected. Failed and interrupted runs are not complete or reusable. Existing output is reused only after configuration, inputs, adapter/executable identity, output contract and every recorded output hash are verified. The canonical FASTA can be handed to the existing catalogue stage. Existing QC and results are not modified.

The full optional-runtime suite passed 277 tests in this environment, including real SPAdes/Tadpole single- and paired-layout fixtures. No candidate discovery, ViReMa, DVG analysis, ranking or biological interpretation was added. The [assembly guide](ASSEMBLY.md) documents configuration, inputs, outputs and limitations. Milestone 3 was published for review in [PR #15](https://github.com/andreiutzx99/atellite-virus-discovery/pull/15); verify its final commit and CI results before treating it as merged.

## Milestone 4: integrated typed artifact workflow

The workflow now connects typed FASTQ validation and prior converted/explicitly validated FASTQ handoffs to registered assembly, canonical contigs, the existing exact-sequence catalogue, declared immutable reference-record imports, local BLAST, occurrence/control summaries, and one consolidated report. Workflow inputs can be local paths, explicitly typed files, or checksum-verified artifacts from an earlier stage. Stage contracts are checked before execution; no arbitrary modules, shell commands, automatic reference curation, or biological interpretation are exposed.

The top-level `workflow.json` links stage order and graph, runtime and Git revision, the configuration digest, input/output artifact descriptors, tool dependencies, state/reuse, warnings/failure, reference-record snapshot IDs, and report paths. The root HTML report summarizes verified stage outputs, execution/reuse, dependencies, inputs, checksums, warnings and failures, and links the final structured/human-readable report. The structured report retains the declared assembly, catalogue, comparison-parameter/result, reference, and occurrence/control summaries. It distinguishes no comparison evidence, a configured-reference no-hit, unavailable dependencies, failed stages, and unexecuted downstream stages. BLAST's short local identifiers are mapped back to original record IDs before evidence is reported; no additional identity/coverage thresholds are configured.

Validation uses deterministic artificial data. Full single-/paired-end SPAdes and Tadpole chains and the supplied-reference match/no-match path are runtime-tested where dependencies are available. These checks demonstrate software execution, handoffs, and reproducible artifact content only; they do not establish biological recovery, sensitivity/specificity, reference completeness, novelty, or scientific interpretation. Existing historical QC/results are not modified or rerun.

### Milestone 4 regression and runtime verification

The archived Milestone 3 `main` baseline (`dd2f8629bd26121c9b8dd8c29420dfdea6b02e6b`) collected 277 tests. With optional tools disabled, 272 passed, none failed, and five live-tool tests were skipped by their optional-runtime guard. With `RUN_OPTIONAL_TOOL_TESTS=1` and the verified local tools available, all 277 passed with no skips.

Milestone 4 adds 17 tests. The complete suite collects 294 tests: the default run has 286 passing, zero failing, and eight optional live-tool tests skipped; the optional-enabled run has 294 passing, zero failing, and zero skipped. The focused M4 file collects 17 tests, with 14 passing and three live-assembler integration tests skipped only when the optional job is disabled. The optional-enabled full run executed and passed real single-/paired-end SPAdes and Tadpole workflows, the real BLAST+ match/unrelated/resume path, and the existing Matplotlib and pysam/samtools runtime tests. All 277 prior tests remain passing in that run.

### Milestone 4 publication record

The publication branch is based on the verified Milestone 3 `main` SHA above and excludes the attached prompt files. Verify the M4 pull request's current head SHA and required Actions checks before merge; after merge, GitHub `main` is authoritative. This handoff records implementation and test results, not a substitute for GitHub's live review/merge receipt.

## Remaining engineering and scientific gaps

Engineering gaps still present include automatic remote acquisition within this workflow, complete dependency pinning/fingerprinting, stronger OS resource containment, broader platform deployment validation and permanent comprehensive runtime-evidence retention. Generic artifact handoffs and artificial fixtures do not establish biological assembly quality or discovery capability.
...
Verified development baseline before Milestone 1: `c5b5eb15d737601b48009a090faef7717c92d54a`; baseline reproduction was 229 passing tests with no skips. Milestone 1 PR #12 merged as `96b6d057d5dcda7c7f1862a3696ef3cae1eb024c`; its recorded main-branch run `36137959715` passed all five jobs. Milestone 2 PR #14 is merged as `9077fe0d2ad367f11a1fc87681b6f3c56a4de2f1`, the base for Milestone 3.

Milestone 3 was published to `feature/milestone-3-generic-assembly` using authenticated Git and opened as [PR #15](https://github.com/andreiutzx99/atellite-virus-discovery/pull/15). Earlier Git-authentication and connector-proxy write failures were resolved for publication; they are not the current repository status. Use the PR and Actions history to identify the final reviewed commit, CI results and merge commit.

The [Actions tests history](https://github.com/andreiutzx99/atellite-virus-discovery/actions/workflows/tests.yml) provides commit-specific run status; an older green run must not be substituted for the final PR head. GitHub `main` is authoritative for merged source; before merging, review the published feature branch and its current SHA. Local environments, ignored data/tools, expiring Actions artifacts and historical prototypes are not part of the released source. The registry is an infrastructure extension point only; it does not establish end-to-end scientific functionality.

## Transfer brief for another platform

> Milestone 4 connects typed, checksummed supplied-FASTQ/assembly/catalogue/reference-comparison/occurrence artifacts with verified reuse and consolidated reports. Check the M4 pull request and Actions for its current review and merge state; after merge, import from GitHub `main`. If continuing from this workspace, read `docs/DEVELOPMENT_HANDOFF.md`, `docs/EXTERNAL_TOOL_ADAPTERS.md`, `docs/ASSEMBLY.md` and `docs/APPLICATION_CAPABILITY_AUDIT.md`. M4 establishes generic software handoffs, not biological discovery. Workflow configuration cannot supply commands or imports. No ViReMa, DVG analysis, candidate classification, ranking or interpretation is implemented. Preserve existing user data and completed QC. Distinguish software behavior, dependencies, missing engineering and scientific validation; do not interpret skipped or unavailable work as negative evidence. Stop for review before authorizing another milestone.
